...
function An_much_better(A, n_iter)
    A_list = [A] 
    for n in 2:n_iter
        Zn = 2*(A/2^(n+1))^2/(1+sqrt(1-(A/2^n)^2))
        A = 2^n*sqrt(4*Zn)
        push!(A_list, A)
    end
    return A_list
...
n_iter = 25
n_iter_plot = 1:25
A2 = 2 * sqrt(2)  # side length of inscribed square in unit circle
pi_approx_kahan = An_much_better(A2, n_iter)

pi_approx = π_polygon.(n_array)
# pi_approx_kahan = π_kahan.(n_array .- 2)
# println(pi_approx_kahan)


plot(n_array, logarithmic_error.(pi_approx),
    marker=(:xcross, 4), label="Approximation using Polygons")
plot!(n_iter_plot, logarithmic_error.(pi_approx_kahan), marker=(:xcross, 4), label="Approximation using Kahan algorithm")
xlabel!("Iterations")
